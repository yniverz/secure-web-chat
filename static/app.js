...
V.search.addEventListener('input', async () => {
    ...
    if (q.length === 8) {
        try {
            ...
            // blob: { id_hash, pubJwk, notifUrl?, notifToken? }
            // Add/merge contact (you choose a name locally)
            const existing = contacts.find(c => c.id_hash === blob.id_hash);
            if (!existing) {
                const newC = { name: blob.name || blob.id_hash.slice(0, 6), id_hash: blob.id_hash, pubJwk: blob.pubJwk, notifUrl: blob.notifUrl, notifToken: blob.notifToken };
                contacts.push(newC);
            ...
            const backCard = { kind: 'contact-card', id_hash: me.id_hash, pubJwk: me.pubJwk, notifUrl: null, notifToken: null, ts: Date.now() };
...
document.getElementById('share-me').addEventListener('click', async () => {
    ...
    const blob = { id_hash: me.id_hash, pubJwk: me.pubJwk, notifUrl: null, notifToken: null, name: me.username };
...
document.getElementById('composer-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Trigger external notification if present
            ...
            try {
                ...
                for (const item of res.items) {
                    if (item.payload?.type === 'msg') {
                        ...
                        if (!c) { c = { name: cc.id_hash.slice(0, 6), id_hash: cc.id_hash, pubJwk: cc.pubJwk, notifUrl: cc.notifUrl, notifToken: cc.notifToken }; contacts.push(c); }
                        else { c.pubJwk = c.pubJwk || cc.pubJwk; if (cc.notifUrl) c.notifUrl = cc.notifUrl; if (cc.notifToken) c.notifToken = cc.notifToken; }
                        await saveEncrypted(me.userKey, LS_KEYS.enc_contacts, contacts);
                        renderContacts(contacts);
                    }
                }
